...
if (!$studentId && isset($_GET['roll'])) {
    $r = $conn->query("SELECT s.Id FROM tblstudent s JOIN tblteacher_course tc ON s.courseId=tc.courseId WHERE tc.teacherId=$teacherId AND s.rollNumber='".mysqli_real_escape_string($conn,trim($_GET['roll']))."'");
    if ($r && $r->num_rows) $studentId = $r->fetch_assoc()['Id'];
}
if (!$studentId) { header("Location: myStudents.php"); exit; }

// Verify teacher owns this student
$owns = $conn->query("SELECT s.Id FROM tblstudent s JOIN tblteacher_course tc ON s.courseId=tc.courseId WHERE tc.teacherId=$teacherId AND s.Id=$studentId")->num_rows;
if (!$owns) { header("Location: myStudents.php"); exit; }

$student   = getStudentFullRecord($conn, $studentId);
if (!$student) { header("Location: myStudents.php"); exit; }
...
<head>
    ...
    <title>Student Analytics - PIMT Teacher</title>
    ...
    <div id="content-wrapper">
        ...
        <div id="page-content" class="fade-in">
            <div class="page-header">
                ...
                <nav class="breadcrumb"><a href="index.php"><i class="fas fa-home"></i></a><i class="fas fa-chevron-right"></i><a href="myStudents.php">Students</a><i class="fas fa-chevron-right"></i><span><?=htmlspecialchars($student['firstName'].' '.$student['lastName'])?></span></nav>
            </div>
...
new Chart(document.getElementById('pieChart').getContext('2d'),{type:'doughnut',data:{labels:labels,datasets:[{data:present,backgroundColor:palette.slice(0,labels.length),borderWidth:2,borderColor:'#0d1b2e',hoverOffset:8}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{position:'bottom',labels:{padding:14,usePointStyle:true}}},cutout:'55%'}});
new Chart(document.getElementById('barChart').getContext('2d'),{type:'bar',data:{labels:labels,datasets:[{label:'Attendance %',data:pcts,backgroundColor:pcts.map(p=>p>=75?'rgba(6,214,160,0.8)':'rgba(239,35,60,0.8)'),borderRadius:6,barThickness:28}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false}},scales:{y:{beginAtZero:true,max:100,grid:{color:'rgba(255,255,255,0.05)'},ticks:{stepSize:25}},x:{grid:{display:false}}}}});
<?php endif; ?>
</script>
</body></html>
