...
if (isset($_GET['Id']) && isset($_GET['action']) && $_GET['action'] == "delete" && isset($_GET['dbKey'])) {
  ...
  if ($dbConnection) {
    ...
    if ($query) {
      resequenceTableIds($dbConnection, 'tblclass', [
        ['table' => 'tblclassarms', 'column' => 'classId'],
        ['table' => 'tblclassteacher', 'column' => 'classId'],
        ['table' => 'tblstudents', 'column' => 'classId'],
        ['table' => 'tblattendance', 'column' => 'classId'],
      ]);
      echo "<script type=\"text/javascript\">
              window.location = ('createClass.php?notice=deleted');
            </script>";
    } else {
