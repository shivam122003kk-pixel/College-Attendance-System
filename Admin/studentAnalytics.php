...
if (!$studentId) { header("Location: viewAllStudents.php"); exit; }

$student = getStudentFullRecord($conn, $studentId);
if (!$student) { header("Location: viewAllStudents.php"); exit; }

$chartData = getStudentSubjectAttendance($conn, $studentId);

// Overall totals
$overallAtt = $conn->query("
    SELECT COUNT(*) as total, SUM(status) as present
...
<head>
    ...
    <title>Student Analytics - <?= htmlspecialchars($student['firstName'].' '.$student['lastName']) ?> - PIMT</title>
    ...
    <style>
        ...
        .stat-pill { display:flex; gap:16px; flex-wrap:wrap; margin-top:12px; }
        .stat-pill-item { background:rgba(255,255,255,0.06); padding:8px 16px; border-radius:20px; font-size:12px; font-weight:600; color:var(--text-light); display:flex; align-items:center; gap:6px; }
    ...
    <div id="content-wrapper">
        ...
        <div id="page-content" class="fade-in">
            ...
            <div class="analytics-hero">
                ...
                <img src="../uploads/students/<?=htmlspecialchars($student['photo'])?>" class="analytics-avatar" alt="Photo">
                <?php else: ?>
                <div class="analytics-avatar-fallback"><?=strtoupper(substr($student['firstName'],0,1).substr($student['lastName'],0,1))?></div>
                <?php endif; ?>
                <div style="flex:1">
                    ...
                    <div style="color:var(--teal);font-size:13px;font-weight:600;margin-bottom:8px"><?=htmlspecialchars($student['deptName']??'General Dept')?></div>
                    <div class="stat-pill">
                        <div class="stat-pill-item"><i class="fas fa-id-badge" style="color:var(--indigo-bright)"></i><?=htmlspecialchars($student['rollNumber'])?></div>
                        <div class="stat-pill-item"><i class="fas fa-book-open" style="color:var(--teal)"></i><?=htmlspecialchars($student['courseCode'])?> - <?=htmlspecialchars($student['courseName'])?></div>
                        <div class="stat-pill-item"><i class="fas fa-venus-mars" style="color:var(--gold)"></i><?=htmlspecialchars($student['gender']??'N/A')?></div>
                    </div>
                </div>
                <div style="text-align:center">
                    ...
                    <?php else: ?>
                    <div style="font-size:18px;color:var(--text-muted)">No data yet</div>
                    <?php endif; ?>
                </div>
            </div>
            ...
            <div class="stats-grid" style="margin-bottom:24px">
                <div class="stat-card indigo">
                    ...
                    <div class="stat-info"><div class="stat-label">Total Classes</div><div class="stat-value"><?=$totalDays?></div></div>
                </div>
                <div class="stat-card success">
                    <div class="stat-icon"><i class="fas fa-check-circle"></i></div>
                    <div class="stat-info"><div class="stat-label">Present</div><div class="stat-value"><?=$presentDays?></div></div>
                </div>
                <div class="stat-card" style="border-color:rgba(239,35,60,0.3)">
                    <div class="stat-icon" style="color:var(--red-alert)"><i class="fas fa-times-circle"></i></div>
                    <div class="stat-info"><div class="stat-label">Absent</div><div class="stat-value" style="color:var(--red-alert)"><?=$absentDays?></div></div>
                </div>
                <div class="stat-card gold">
                    <div class="stat-icon"><i class="fas fa-book"></i></div>
                    <div class="stat-info"><div class="stat-label">Subjects</div><div class="stat-value"><?=count($student['subjects'])?></div></div>
                </div>
            </div>
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:24px;margin-bottom:24px">
                ...
                <div class="cams-card" style="margin-bottom:0">
                    ...
                    <div class="card-body" style="position:relative;height:300px;display:flex;justify-content:center">
                        ...
                        <?php else: ?>
                        <div class="empty-state" style="padding:40px"><div class="empty-icon"><i class="fas fa-chart-pie"></i></div><div class="empty-title">No subject data yet</div></div>
                        <?php endif; ?>
                    </div>
                </div>
                ...
                <div class="cams-card" style="margin-bottom:0">
                    ...
                    <div class="card-body" style="position:relative;height:300px">
                        ...
                        <?php else: ?>
                        <div class="empty-state" style="padding:40px"><div class="empty-icon"><i class="fas fa-chart-bar"></i></div><div class="empty-title">No data yet</div></div>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
            ...
            <div class="cams-card">
                ...
                <div class="card-body">
                    ...
                    <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(260px,1fr));gap:14px">
                        ...
                        <div class="subject-att-card">
                            <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:10px">
                                <div>
                                    <div style="font-weight:700;font-size:14px"><?=htmlspecialchars($sb['subjectName'])?></div>
                                    <div style="font-size:11px"><span class="badge badge-indigo"><?=$sb['subjectCode']?></span> <?=htmlspecialchars($sb['semester']??'')?></div>
                                </div>
                                <div style="text-align:right">
                                    <div style="font-weight:800;font-size:22px;color:<?=$fillColor?>"><?=$pct!==null?$pct.'%':'N/A'?></div>
                                </div>
                            </div>
                            <div class="subject-att-bar-bg">
                                <div class="subject-att-bar-fill" style="width:<?=$pct??0?>%;background:<?=$fillColor?>"></div>
                            </div>
                    ...
                    <?php else: ?>
                    <div class="empty-state"><div class="empty-icon"><i class="fas fa-book"></i></div><div class="empty-title">No subjects enrolled yet</div></div>
                    <?php endif; ?>
                </div>
            </div>
        </div>
...
new Chart(document.getElementById('subjectBarChart').getContext('2d'), {
    ...
    data: {
        ...
        datasets: [{
            label: 'Attendance %',
            data: pcts,
            backgroundColor: pcts.map(p => p >= 75 ? 'rgba(6,214,160,0.8)' : 'rgba(239,35,60,0.8)'),
            borderRadius: 6,
            barThickness: 30
        }]
    },
    options: {
        ...
        plugins: {
            ...
            annotation: { annotations: { line: { type:'line', yMin:75, yMax:75, borderColor:'rgba(255,200,68,0.7)', borderWidth:2, borderDash:[6,4] } } }
        },
        scales: {
            y: { beginAtZero: true, max: 100, grid: { color:'rgba(255,255,255,0.05)' }, ticks: { stepSize: 25 } },
