...
<head>
    ...
    <title>My Analytics - PIMT Student</title>
    ...
    <style>
        ...
        .analytics-avatar-fallback{width:96px;height:96px;border-radius:18px;background:linear-gradient(135deg,var(--indigo),var(--teal));display:flex;align-items:center;justify-content:center;font-size:30px;font-weight:800;color:#fff;}
        ...
        .subject-att-bar-fill{height:8px;border-radius:6px;}
    </style>
    ...
    <div id="content-wrapper">
        ...
        <div id="page-content" class="fade-in">
            <div class="page-header">
                <h1 class="page-title">My <span>Attendance Analytics</span></h1>
                <nav class="breadcrumb"><a href="index.php"><i class="fas fa-home"></i></a><i class="fas fa-chevron-right"></i><span>My Analytics</span></nav>
            </div>
            ...
            <div class="analytics-hero">
                ...
                <div style="flex:1">
                    <h2 style="font-family:'Outfit',sans-serif;font-size:24px;font-weight:800;margin-bottom:4px"><?=htmlspecialchars($student['firstName'].' '.$student['lastName'])?></h2>
                    ...
                    <div style="display:flex;gap:10px;flex-wrap:wrap">
                        ...
                        <div style="background:rgba(255,255,255,0.06);padding:6px 14px;border-radius:20px;font-size:12px;font-weight:600"><i class="fas fa-book-open" style="color:var(--gold);margin-right:6px"></i><?=htmlspecialchars($student['courseCode'])?> - <?=htmlspecialchars($student['courseName'])?></div>
                    </div>
                </div>
                <div style="text-align:center">
                    ...
                    <?php if($overallPct<75): ?><div style="margin-top:8px;background:rgba(239,35,60,0.15);border:1px solid rgba(239,35,60,0.3);border-radius:8px;padding:6px 12px;font-size:12px;color:#ef233c;font-weight:600"><i class="fas fa-exclamation-triangle"></i> Below 75% threshold!</div><?php endif; ?>
                    <?php else: ?><div style="font-size:18px;color:var(--text-muted)">No records yet</div><?php endif; ?>
                </div>
            </div>
            <?php endif; ?>
            ...
            <div class="stats-grid" style="margin-bottom:24px">
                ...
                <div class="stat-card" style="border-color:rgba(239,35,60,.3)"><div class="stat-icon" style="color:var(--red-alert)"><i class="fas fa-times-circle"></i></div><div class="stat-info"><div class="stat-label">Absent</div><div class="stat-value" style="color:var(--red-alert)"><?=$absentDays?></div></div></div>
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:24px;margin-bottom:24px">
                <div class="cams-card" style="margin-bottom:0">
                    ...
                    <div class="card-body" style="position:relative;height:280px;display:flex;justify-content:center">
                        <?php if(!empty($chartData['labels'])): ?><canvas id="pieChart"></canvas>
                ...
                <div class="cams-card" style="margin-bottom:0">
                    <div class="card-header"><div class="card-title"><i class="fas fa-chart-bar"></i> % per Subject</div></div>
                    <div class="card-body" style="position:relative;height:280px">
                        <?php if(!empty($chartData['labels'])): ?><canvas id="barChart"></canvas>
            ...
            <!-- Subject Breakdown -->
            <div class="cams-card">
                <div class="card-header"><div class="card-title"><i class="fas fa-book"></i> Subject-wise Breakdown</div></div>
                <div class="card-body">
                    ...
                    <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(260px,1fr));gap:14px">
                        ...
                        <div class="subject-att-card">
                            <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:10px">
                                <div><div style="font-weight:700;font-size:14px"><?=htmlspecialchars($sb['subjectName'])?></div><span class="badge badge-indigo"><?=$sb['subjectCode']?></span></div>
                            ...
                            <div style="margin-top:8px;font-size:11px;color:#ef233c;font-weight:600"><i class="fas fa-exclamation-triangle"></i> Below mandatory 75% threshold</div>
                            <?php endif; ?>
                        </div>
                        <?php endforeach; ?>
                    </div>
                    <?php else: ?><div class="empty-state"><div class="empty-icon"><i class="fas fa-book"></i></div><div class="empty-title">No subjects found</div></div><?php endif; ?>
                </div>
            </div>
        </div>
...
const palette=['#06d6a0','#4f63d2','#f59e0b','#ef233c','#60a5fa','#a78bfa','#fb923c'];
new Chart(document.getElementById('pieChart').getContext('2d'),{type:'doughnut',data:{labels:labels,datasets:[{data:present,backgroundColor:palette.slice(0,labels.length),borderWidth:2,borderColor:'#0d1b2e',hoverOffset:8}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{position:'bottom',labels:{padding:14,usePointStyle:true}}},cutout:'60%'}});
new Chart(document.getElementById('barChart').getContext('2d'),{type:'bar',data:{labels:labels,datasets:[{label:'Attendance %',data:pcts,backgroundColor:pcts.map(p=>p>=75?'rgba(6,214,160,0.8)':'rgba(239,35,60,0.8)'),borderRadius:6,barThickness:30}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false}},scales:{y:{beginAtZero:true,max:100,grid:{color:'rgba(255,255,255,0.05)'},ticks:{stepSize:25}},x:{grid:{display:false}}}}});
<?php endif; ?>
</script>
</body></html>
